...
class Predictor:
    ...
    def predict(self, data: dict):
        ...
        bin_size = 15
        meldtijd_bin = (data['stm_sap_meldtijd'] // bin_size) * bin_size
        aanntpl_tijd_bin = (data['stm_aanntpl_tijd'] // bin_size) * bin_size
        progfh_in_duur_bin = (data['stm_progfh_in_duur'] // bin_size) * bin_size

        # Bereken tijdsverschil en spitsuur-indicator
        tijd_verschil_meld_aanntpl = data['stm_aanntpl_tijd'] - data['stm_sap_meldtijd']
        uur_van_dag = (data['stm_sap_meldtijd'] // 60)
        ...
        lr_data = pd.DataFrame([{
            'stm_sap_meldtijd': data['stm_sap_meldtijd'],
            'stm_sap_melddatum': data['stm_sap_melddatum'],
            'stm_aanntpl_tijd': data['stm_aanntpl_tijd'],
